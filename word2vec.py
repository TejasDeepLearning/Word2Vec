...
# unfortunately these work in different ways
...
# choose which remove punctuation function to use
if sys.version.startswith('2'):
    remove_punctuation = remove_punctuation_2
else:
    remove_punctuation = remove_punctuation_3

# function to get the wikipedia data
def get_wiki():
    # vocabulary size
    V = 20000
    files = glob("/Users/tejas/Documents/Deep Learning/Lazy Programmer Courses/Natural Language Processing with Deep Learning in Python/Section 2/enwiki-preprocessed/enwiki*.txt")
    all_word_counts = {}
    for f in files:
    ...
    for f in files: 
        for line in open(f, encoding='utf-8'):
            if line and line[0] not in '[*-|=\{\}':
                s = remove_punctuation(line).lower().split()
                if len(s) > 1:
                    # if a word is not nearby another word, there won't have any context, and hence nothing to train! 
...
def train_model(savedir):
    ...
    for epoch in range(epochs):
        ...
        # print stuff so we dont stare at a blank screen
        dt = datetime.now() - t0
